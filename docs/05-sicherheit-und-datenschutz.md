...
Hinweis zur Quellenlage: Die Faktenprüfung (`docs/recherche/verifikation.json`, Stand 3. September 2026; 18 Aussagen bestätigt, 12 präzisiert) ist eingearbeitet [70]. Sie deckt ausschließlich technische Aussagen der iOS- und Android-Berichte ab; für dieses Dokument relevant sind die Bestätigungen zu watchOS (Abschnitt 2.7) und die Präzisierungen zu Controls und Foreground Services. Rechtliche Aussagen (Google-Play-Richtlinien, EU-Rechtstexte, Schweizer Recht, MDR/MDCG, BIS/ANSSI) waren nicht Gegenstand der Prüfung; sie bleiben mit "(unverifiziert)" gekennzeichnet und müssen vor Veröffentlichung gegen die genannte Quelle geprüft werden. Das Verschlüsselungsformat der Audiodateien folgt der Nachrecherche zum Audio-Container vom 3. September 2026 (D4: Chunk-AEAD statt secretstream) [71]. Dieses Dokument ist keine Rechtsberatung.
...
|---|---|---|
| Audiodateien (AAC-LC in M4A, D4) | Chunkweise AEAD mit wahlfreiem Zugriff nach Cryptomator-Muster: Dateikopf mit Kopf-Nonce und dem mit dem Master-Key gewrappten Dateischlüssel; Inhalt in 32-KiB-Chunks, je Chunk eigene zufällige Nonce und Auth-Tag (`crypto_aead_xchacha20poly1305_ietf`, alternativ AES-256-GCM); Zusatzdaten (AAD) = Chunk-Nummer (64 Bit) + Kopf-Nonce, damit Umsortieren, Duplizieren und Vertauschen zwischen Dateien auffallen; Chunk-Anzahl im authentifizierten Kopf, damit auch Abschneiden am Chunk-Ende erkannt wird (eigene Ergänzung, secretstream löste das über `TAG_FINAL`). Jeder Chunk ist unabhängig entschlüsselbar, Chunk-Index = Position / 32 768; so bleibt die Wiedergabe seekbar, ohne Klartext auf die Platte zu schreiben [55][71][72][73] | libsodium 1.0.22 über `package:sodium` 4.1.0+1 [4] |
...
| Schlüssel-Wrapping in der App | `crypto_secretbox` (XSalsa20-Poly1305) mit dem Master-Key; der gewrappte Dateischlüssel steht im Kopf der Audiodatei, eine Kopie in `recording.file_key_wrapped` | libsodium [4] |
...
| Transfer-Container (Gerätewechsel, Backup) | X25519 + HKDF (Gerätewechsel) bzw. Argon2id (Backup) zum Container-Schlüssel, danach `crypto_secretstream_xchacha20poly1305` in 64-KiB-Chunks, letzter Chunk `TAG_FINAL`; erkennt Abschneiden, Umsortieren, Duplizieren und Manipulation [1]. secretstream ist strikt sequenziell und deshalb nur hier richtig: Ein Container wird am Stück gelesen, eine Audiodatei nicht (D4, D6) [71] | libsodium |

Warum Chunk-AEAD statt secretstream für Audiodateien: secretstream ist strikt sequenziell; für einen Sprung an Minute zwei müsste die Datei von vorn bis dorthin entschlüsselt werden, oder man führte pro Chunk einen eigenen Stream, was auf dasselbe Format hinausläuft [1][71]. Das Cryptomator-Schema (Kopf mit Nonce und gewrapptem Dateischlüssel, 32-KiB-Chunks mit je eigener Nonce und Tag, Chunk-Nummer plus Kopf-Nonce als Zusatzdaten) ist seit Jahren im Produktionsbetrieb, durch Cure53 geprüft und in Java und Swift referenzimplementiert; die App baut es mit libsodium-Primitiven nach, statt `cryptolib` (AGPLv3) einzubinden [55][62][72]. Bewusst nicht gewählt: reines AES-CTR mit Offset-Zähler, wie es Media3 (`AesCipherDataSource`) und Signal Android (`ModernDecryptingPartInputStream`) für seekbare Medien nutzen, weil es keine Integritätsprüfung bietet [73]. Warum XChaCha20-Poly1305 statt AES-GCM als Chunk-Primitiv: Die 192-Bit-Nonce erlaubt zufällige Nonces je Chunk ohne Zählerverwaltung, die Geschwindigkeit ist auch ohne Hardware-AES konstant, und Standard Notes zeigt das Verfahren im Produktionsbetrieb [56]. AES-256-GCM (CryptoKit, `javax.crypto`) ist nach D4 die gleichwertige Alternative und bleibt dort gesetzt, wo die Hardware selbst rechnet (Keystore-Wrapping); entscheidend ist die saubere Schlüsselhierarchie, nicht die Wahl zwischen den beiden AEADs [55].
...
Master-Key MK (256 Bit, CSPRNG, nie auf Platte im Klartext)
        ...
        └──▶ DEK je Aufnahme (256 Bit) ──▶ Chunk-AEAD-Audiodatei (M4A-Inhalt)
                                          Kopf: Kopf-Nonce + gewrappter DEK
                                          Chunk n: Nonce_n | AEAD(DEK, AAD = n ‖ Kopf-Nonce) | Tag_n
                                          (Kopie des gewrappten DEK in `recording.file_key_wrapped`;
                                           Wellenform, 100 Werte, liegt verschlüsselt bei den Metadaten)
...
1. Pro Aufnahme ein eigener Dateischlüssel (DEK), der die Datei nie im Klartext verlässt: Im Dateikopf liegt er mit dem Master-Key gewrappt, damit die Datei selbsttragend ist (Cryptomator-Muster) und im Transfer-Container unverändert mitreisen kann; die Kopie in der Datenbank dient Index und Löschung. Löschen einer Aufnahme heißt DEK vernichten (Abschnitt 3.4); Schlüsselrotation berührt nur die Wrapping-Ebene, das heißt Kopf und Datenbankzeile werden neu gewrappt, die Chunks bleiben unangetastet.
...
**Empfehlung: Die App-Sperre bindet den Schlüssel an "Biometrie oder Gerätecode", nicht an "nur aktuelle Biometrie".** Begründung: Mit `.biometryCurrentSet` (iOS) bzw. `setInvalidatedByBiometricEnrollment` (Android, Standardverhalten) wird der Schlüssel ungültig, sobald ein Fingerabdruck hinzukommt [8][13]. Für ein Tagebuch ohne Server-Recovery wäre das ein Datenverlust durch eine alltägliche Handlung. Dass das kein theoretischer Fall ist, zeigt eine archivierte Play-Store-Rezension zu Easy Diary: Nach dem Hinzufügen eines weiteren Fingerabdrucks verlangte die App eine PIN, die der Nutzer nie gesetzt hatte, und sperrte ihn aus seinem eigenen Tagebuch aus (16. August 2022) [74][75]. Deshalb:
...
5. Die App zeigt nach dem Export an, wo die Containerdatei liegt, und lässt beim Import jede Datei manuell wählen. Die häufigsten echten Store-Beschwerden zu Tagebuch-Backups sind nicht Kryptografie, sondern Auffindbarkeit: Nutzer wissen nicht, wo die Backup-Datei gespeichert wurde, ein Betriebssystem-Update (Android 13, Scoped Storage) bricht die Backup-Funktion, oder es gibt keinen Weg, eine ältere Datei auszuwählen [74][75]. Weil der Container über das System-Teilen-Blatt an einen vom Nutzer gewählten Ort geht (D6), hängt die App nicht an einem festen Pfad, den ein Update entwerten könnte.
...
1. **Kein Klartext auf der Uhr.** Aufnahme über `AVAudioEngine.inputNode` (auf watchOS ab Version 4 verfügbar) [66] als PCM im Speicher, AAC-LC-Kodierung im Speicher (`AVAudioConverter`) und chunkweise AEAD in 32-KiB-Chunks im selben Dateiformat wie auf dem Telefon (D4, Abschnitt 2.1). `AVAudioRecorder` wird nicht verwendet, obwohl es auf watchOS verfügbar wäre (Faktenprüfung: bestätigt) [70], weil es direkt in eine Datei schreibt [69]. Ob libsodium (oder als Ersatz CryptoKit `AES.GCM`/`ChaChaPoly` mit demselben Chunk-Layout) und die AAC-Kodierung im Speicher auf watchOS in akzeptabler Größe laufen, klärt der Spike; der spätere Opus-Pfad (libopus als xcframework) käme auf der Uhr erst danach.
...
3. **Übertragung nur als Chiffrat.** `transferFile` überträgt die fertige Datei (Kopf mit Schlüssel-Kennung des Schreibschlüssels, versiegelter DEK statt des mit dem Master-Key gewrappten, Kopf-Nonce, Chunk-AEAD-Nutzlast). Die Übertragung läuft asynchron auf einem Hintergrund-Thread, gegebenenfalls gedrosselt und mit Warteschlange (Faktenprüfung: bestätigt) [70]. Das iPhone bewegt die Datei im Delegate synchron in seinen Container [65]; der temporäre Pfad ist damit unkritisch, weil er nie Klartext enthält. Entsiegelt wird der DEK erst, wenn die App entsperrt ist; dann wird er mit dem Master-Key per `crypto_secretbox` neu gewrappt (Abschnitt 2.2), die versiegelte Kopie gelöscht und die Transkription gestartet. Die Uhr löscht ihre Kopie nach bestätigter Zustellung.
...
Nebeneffekt: Derselbe Schreibschlüssel ist ein Kandidat für die offene Frage "Aufnahme bei gesperrter App" in `03-produktkonzept.md` (Controls, Widgets). Zum Versiegeln reicht der öffentliche Schlüssel, der ohne Authentifizierung vorliegen darf; der Master-Key wird erst beim nächsten Entsperren gebraucht. D5 bleibt unberührt, weil der DEK am Ende wie jeder andere mit dem Master-Key gewrappt ist. Zwei Präzisierungen aus der Faktenprüfung gehören dazu [70]: Controls laufen standardmäßig auch bei gesperrtem Gerät (`authenticationPolicy` = `alwaysAllowed`), Apple empfiehlt aber, bei Bedarf Authentifizierung zu verlangen und Text im gesperrten Zustand zu redigieren; und ein Aufnahme-Intent muss `AudioRecordingIntent` adoptieren und eine Live Activity starten, sonst stoppt die Aufnahme. Ob ein Kaltstart der Aufnahme aus dem Hintergrund per Intent überhaupt funktioniert, ist durch Apple nicht belegt und wird in einem Forumsthread verneint; das ist vor jeder Planung auf dem Gerät zu testen.
...
Geheimnisse liegen im Prozess nur in `Uint8List`/`SecureKey`, werden nach Gebrauch überschrieben und beim Wechsel in den Hintergrund verworfen; keine `String`-Repräsentation, keine Logs mit Inhalten, keine Absturzberichte mit Speicherabbildern (MASTG-TEST-0011 und -0060) [21]. Entschlüsseltes Audio wird für die Wiedergabe chunkweise (32 KiB) in den Speicher geholt, nie als temporäre Datei abgelegt (`04-technik-architektur.md`, Abschnitt 3). Das gilt auch für die Systemintegration: Auf Android läuft die Wiedergabe in einem `MediaSessionService` mit ExoPlayer und einer eigenen `DataSource`, die je Leseposition nur den betroffenen Chunk entschlüsselt (Pflicht ab Android 17, weil Hintergrundaudio ohne Foreground Service stumm unterdrückt wird); auf iOS dekodiert `AVAudioEngine` mit `AVAudioPlayerNode` aus entschlüsselten Chunks über `AudioFileOpenWithCallbacks`/`AVAudioConverter` [71][76]. Beide Wege kommen ohne Datei-URL aus, weshalb kein Klartext-Pfad existiert, den ein Systemplayer, eine Sperrbildschirm-Steuerung oder ein Absturzbericht preisgeben könnte. Die Wellenform (100 Pegelwerte) wird bei der Aufnahme berechnet und verschlüsselt mit den Metadaten gespeichert, damit sie später nicht aus einer Volldekodierung nachgerechnet werden muss [71].
...
| Aktion | Was passiert |
|---|---|
| Nur Audio eines Eintrags löschen | gewrappter DEK aus `recording.file_key_wrapped` entfernen (SQLCipher `secure_delete`), Datei samt Kopf unlinken; Transkript und Wellenform-Metadaten bleiben, sofern nicht mit gelöscht. Restrisiko: Der unlinkte Dateikopf mit dem gewrappten DEK kann auf dem Flash überdauern; nutzbar ist er nur zusammen mit dem Master-Key, also nur für die Person, die die App ohnehin entsperren kann. Ob der Kopf deshalb unter einem zusätzlichen, nur in der Datenbank gehaltenen Zwischenschlüssel gewrappt wird, entscheidet der Spike (offene Frage 13) |
...
Der Wettbewerb macht die Frage dringlicher: DailyVox (iOS, kostenlos, MIT) hat im August 2026 mit Version 1.10.0 Deutsch hinzugefügt und mit 1.11.0 den Cloud-Fallback vollständig entfernt; damit gibt es auf iOS einen offenen, kostenlosen, deutschsprachigen und vollständig lokalen Wettbewerber, eine Android-Version fehlt trotz dokumentierter Nachfrage [74][77]. Auf iOS unterscheidet sich die App also nicht mehr durch "lokal" und "offen", sondern durch das, was dieses Dokument beschreibt: Verschlüsselung auf dem Gerät (Apple Journal etwa legt Einträge in einer unverschlüsselten SQLite-Datenbank ab [74]), das verschlüsselte Backup mit Wiederherstellungsprobe, den QR-Gerätewechsel auch zwischen den Plattformen und den nachprüfbaren Netz-Verzicht. Ein proprietärer Kern würde genau diesen Vergleich gegen ein MIT-Projekt verlieren.
...
Die verbindlichen Entscheidungen D1, D4 und D5 setzen auf libsodium und SQLCipher, also gebündelte Industriestandard-Kryptografie. Der Rechtsbericht empfiehlt aus Vereinfachungsgründen reine Betriebssystem-Kryptografie. Seit der Umstellung auf Chunk-AEAD (D4) ließe sich die lokale Audiodatei tatsächlich auch mit Plattform-Primitiven bauen (CryptoKit `AES.GCM`/`ChaChaPoly`, Android `javax.crypto` mit Keystore-Schlüssel) [71][78]; der Transfer-Container (secretstream, X25519, Argon2id) und SQLCipher bleiben aber gebündelte Kryptografie, sodass die Dokumentationspflicht ohnehin entsteht. **Empfehlung: bei libsodium/SQLCipher bleiben und den Dokumentationsaufwand tragen**, weil er einmalig und gebührenfrei ist und ein plattformidentisches, auditierbares Datei- und Containerformat mehr wert ist als der Verzicht auf die Deklaration: `ITSAppUsesNonExemptEncryption = YES` mit Einordnung als Ausnahme (Standardalgorithmen), französische Deklaration bei der ANSSI (nach Décret 2007-663, gebührenfrei, unverifiziert) [50], und der jährliche BIS-Self-Classification-Report bis 1. Februar (unverifiziert) [49]. Für Android gibt es keinen Fragebogen, dieselbe Einordnung gilt. Dieser Punkt braucht vor der ersten Einreichung eine Prüfung durch eine auf Exportkontrolle spezialisierte Stelle (offene Frage).
...
12. Uhr-Aufnahmen (Abschnitt 2.7): Landen Watch-App-Daten in iPhone-Backups, und lassen sie sich ausschließen? Laufen libsodium (oder CryptoKit mit demselben Chunk-Layout) und die AAC-Kodierung im Speicher in tragbarer Größe auf watchOS? Gibt es für Wear OS einen Transport ohne Google-Server? Erst danach wird die Funktion eingeplant.
13. Chunk-AEAD-Dateiformat (Abschnitt 2.1): endgültiges Kopf-Layout (Formatversion, Kopf-Nonce, gewrappter DEK, Chunk-Anzahl oder Klartextlänge gegen Abschneiden), Testvektoren im Repository, Verhalten bei beschädigtem Einzel-Chunk (Rest abspielbar oder Abbruch), und ob der Kopf wegen des Crypto-Shreddings (Abschnitt 3.4) unter einem zusätzlichen, nur in der Datenbank gehaltenen Zwischenschlüssel gewrappt wird.
...
70. Faktenprüfung der Rechercheberichte (30 technische Aussagen, 18 bestätigt, 12 präzisiert; Stand 3. September 2026): `docs/recherche/verifikation.json` in diesem Repository; Primärquellen darin u. a. https://developer.apple.com/documentation/appintents/audiorecordingintent, https://developer.apple.com/documentation/appintents/appintent/authenticationpolicy, https://developer.apple.com/forums/thread/815725, https://developer.apple.com/documentation/watchconnectivity/wcsession/transferfile(_:metadata:)
71. Nachrecherche Audio-Container, Aufnahme-Pipeline und Wiedergabe-Architektur (Stand 3. September 2026): `docs/recherche/nachrecherche-audio-container.md` in diesem Repository (Abschnitte 3.1 bis 3.4, 5 und 6: Chunk-AEAD statt secretstream, Wiedergabe ohne Klartext-Tempdatei, Wellenform bei der Aufnahme)
72. https://github.com/cryptomator/docs/blob/develop/docs/security/vault.mdx (Dateikopf, 32-KiB-Chunks, AAD = Chunk-Nummer + Header-Nonce); https://github.com/cryptomator/cryptolib/releases (2.2.2, AGPLv3/kommerziell); https://github.com/cryptomator/cryptolib-swift; https://github.com/jedisct1/libsodium-doc/blob/master/secret-key_cryptography/aead/chacha20-poly1305/xchacha20-poly1305_construction.md (`crypto_aead_xchacha20poly1305_ietf`, 192-Bit-Nonce)
73. https://github.com/androidx/media/blob/release/libraries/datasource/src/main/java/androidx/media3/datasource/AesCipherDataSource.java und .../AesFlushingCipher.java (AES/CTR, Zähler aus Offset); https://github.com/signalapp/Signal-Android/blob/main/core/util/src/main/java/org/signal/core/util/crypto/ModernDecryptingPartInputStream.java (AES/CTR, keine Integritätsprüfung auf Dateiebene)
74. Nachrecherche Nutzerstimmen und Store-Kennzahlen (Stand 3. September 2026): `docs/recherche/nachrecherche-nutzerstimmen-stores.md` in diesem Repository (Abschnitte 1.3, 3 und 4: Backup-Auffindbarkeit, Biometrie-Aussperrung, DailyVox, Apple Journal unverschlüsselt)
75. https://github.com/hanjoongcho/aaf-easydiary/issues/197 (Aussperrung nach neuem Fingerabdruck, 16. August 2022); https://github.com/hanjoongcho/aaf-easydiary/issues/184 (Speicherort der Backup-Datei nicht auffindbar); https://github.com/hanjoongcho/aaf-easydiary/issues/200 (Backup nach Android 13 blockiert); https://github.com/hanjoongcho/aaf-easydiary/issues/111 (Backup-Datei nicht manuell wählbar); https://github.com/billthefarmer/diary/issues/236
76. https://developer.android.com/about/versions/17/changes/bg-audio (Hintergrundaudio ohne Foreground Service stumm unterdrückt); https://developer.android.com/media/media3/session/background-playback (`MediaSessionService`); https://developer.apple.com/documentation/avfaudio/avaudioplayernode/schedulebuffer(_:completionhandler:); https://developer.apple.com/documentation/mediaplayer/mpremotecommandcenter
77. https://github.com/intrepidkarthi/dailyvox (MIT; Deutsch ab v1.10.0, Cloud-Fallback entfernt in v1.11.0, August 2026); https://github.com/intrepidkarthi/dailyvox/issues/4 (Android-Nachfrage, 3. August 2026)
78. https://developer.apple.com/documentation/cryptokit/chachapoly und https://developer.apple.com/documentation/cryptokit/aes/gcm (Plattform-AEADs mit 96-Bit-Nonce als Alternative zu libsodium für lokale Dateien)
