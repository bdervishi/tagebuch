...
- Die App ist gut positioniert, weil die meisten Beschwerden aus Architekturentscheidungen der Wettbewerber folgen (Server, Konto, Cloud-Transkription, Abo-Gating), nicht aus Detailfehlern. Wer ohne Server baut, erbt diese Probleme nicht, muss dafür aber Backup, Transfer und Aufnahme-Robustheit über jeden Zweifel erhaben machen. Eine Einschränkung seit August 2026: DailyVox ist auf iOS ein kostenloser, deutschsprachiger, vollständig lokaler Wettbewerber (v1.10.0 Deutsch, v1.11.0 ohne Cloud-Fallback) [10][72]; der Unterschied liegt dort in Verschlüsselung im Alltag, Backup und QR-Gerätewechsel, Erinnerungslogik und Produktreife, plattformübergreifend in Android, wo DailyVox trotz dokumentierter Nachfrage nicht existiert [32].
...
Die Nutzerfeedback-Recherche lief am 3. September 2026 unter einer starken Einschränkung: App Store, Google Play, Reddit, Hacker News, Trustpilot, heise, Kuketz-Blog und die Apple-Community waren aus der Recherche-Umgebung nicht erreichbar (D15). Direkt lesbar war GitHub (Issue-Tracker, Discussions, READMEs); ergänzend liegen Suchsnippets aus den Wettbewerbs- und Innovationsrecherchen vor. Eine Nachrecherche am selben Tag (`docs/recherche/nachrecherche-nutzerstimmen-stores.md`) scheiterte erneut an der Sperre der Store-, Reddit- und Forenseiten, brachte aber drei neue Quellentypen ein: vom Entwickler von Easy Diary als GitHub-Issues archivierte Play-Store-Rezensionen mit Datum, deutschsprachige Threads in den Apple Developer Forums zum Systemdiktat und neun deutschsprachige App-Store-Rezensionsfragmente aus Suchsnippets (Day One, Diarium; ohne Nutzername, Version und Datum, deshalb nur als Tonalitätshinweis verwendet) [72].
...
3. **Die Faktenprüfung deckt nur technische Aussagen ab.** `docs/recherche/verifikation.json` (Stand 3. September 2026) prüft 30 Aussagen aus den Technikberichten zu iOS und Android; 18 wurden bestätigt, 12 korrigiert oder präzisiert [69]. Markt-, Preis-, Benchmark- und Nutzerzahlen waren nicht Gegenstand der Prüfung. Für dieses Dokument sind fünf Ergebnisse relevant und eingearbeitet: die whisper.cpp-Messung auf dem Samsung Note10 (Abschnitt 3.5, bestätigt und präzisiert), der Apple-Watch-Benchmark (Abschnitt 3.5, bestätigt), der Speicherbedarf des Parakeet-Modells (Abschnitt 3.5, präzisiert), die Moonshine-Modelldaten (Abschnitt 3.5, bestätigt) und die Versionsabhängigkeit der von `SpeechTranscriber` gemeldeten Sprachen (Abschnitt 3.4, präzisiert). Alle übrigen Zahlen, die im Recherchebericht als `mustVerify` markiert oder aus Suchsnippets rekonstruiert sind und nicht in `verifikation.json` stehen, tragen weiterhin den Zusatz "(unverifiziert)". Direkt auf GitHub oder in den Apple Developer Forums gelesene Zitate sind mit Datum und URL belegt.

Eine Nachrecherche mit tatsächlichem Store- und Reddit-Zugriff steht in Abschnitt 8 als offene Aufgabe; das Abrufmuster dafür (RSS-Rezensionsfeed, Lookup-API, `google-play-scraper`, App-IDs) ist in der Nachrecherche dokumentiert [72].
...
**Konsequenz:** Export ist eine Vertrauensbedingung, kein Komfortmerkmal. Die App liefert Markdown, JSON und PDF kostenlos (D9), Audio als M4A-Dateien (AAC-LC, D4) mit, die jedes Betriebssystem ohne Zusatzsoftware abspielt [73], und der Export sollte vor dem ersten Eintrag sichtbar sein (Onboarding-Hinweis "So bekommst du alles wieder heraus").
...
- EchoVault (Android, Store-Snippet): *"Data loss has also been reported, with some users losing all entries."* (unverifiziert) [63][72].

Die einzigen echten Play-Store-Rezensionen der Stichprobe, vom Entwickler von Easy Diary als Issues archiviert, drehen sich fast alle um Backup und Zugang [72][74]:

- *"backup to device can't be seen where it is saved on interbal device"* (28. Dezember 2021): Der Nutzer findet die Backup-Datei auf dem Gerät nicht, was bei Gerätewechsel oder Reparatur entscheidend ist.
- *"Insufficient permission for the request feature"* (11. November 2022): Nach dem Update auf Android 13 ist Backup auf Drive und Gerät auf einem Pixel 5 komplett blockiert; der Nutzer will die App erst wieder verwenden, *"once the backup functionality is restored"*. Ein Betriebssystem-Update hat das Backup gebrochen.
- *"Recently I foolishly added another fingerprint to my device lock and all apps that relied on fingerprint unlock had to have their locks reset."* (16. August 2022): Die App verlangt danach eine PIN, die der Nutzer nie gesetzt hat; er ist aus dem eigenen Tagebuch ausgesperrt.
- *"I have a backup file from 5th Jan 2020. But there is no option to choose backup file manually."* (22. März 2020).
- Dasselbe Muster in anderen Projekten: Ein deutschsprachiger Nutzer von billthefarmer/Diary fragt *"Ho to restore an Backup? There is noch menu or button"* (7. November 2024) [75]; bei StoryPad wurden nach einer Neuinstallation nur die Einträge von 2025 wiederhergestellt, *"2026 records failed despite being synced to Google Drive (~400 MB)"* (11. März 2026) [76].
...
**Konsequenz:** Für eine App ohne Server ist das Thema Backup und Transfer die verwundbarste Stelle. D6 legt fest: dieselbe Containerdatei für Gerätewechsel und Backup, mit Passphrase, monatlicher Erinnerung und Erklärung, dass Betriebssystem-Backups die Daten nicht wiederherstellen. Ergänzend empfohlen: ein Wiederherstellungstest ("Backup prüfen") direkt nach dem Erstellen, Aufnahme chunkweise verschlüsselt auf Disk (Chunk-AEAD, D4), Schema-Migrationen mit Vorab-Sicherung. Aus den Store-Rezensionen kommen drei weitere Pflichten: Die App zeigt nach dem Erstellen sichtbar an, wo die Containerdatei liegt, und bietet sie über das Teilen-Blatt an, statt sie in einem App-Ordner zu verstecken; die Wiederherstellung lässt eine Datei manuell auswählen und nennt Datum und Umfang, bevor sie importiert; und eine Änderung der Biometrie darf niemanden aussperren, weshalb der Gerätecode immer als Rückfall funktioniert und ein biometrisch gebundener Schlüssel (D5, `.biometryCurrentSet` bzw. Invalidierung bei neuer Registrierung) nur mit erklärter Passphrase-Wiederherstellung (D6) eingesetzt wird.
...
Verschlüsselung ist bei Tagebuch-Projekten die am stärksten nachgefragte Eigenschaft: bei billthefarmer/Diary sind "Encrypt diary option", "Encrypted Diary" und "password access" die reaktionsstärksten Issues [13]; bei Journiv steht *"The data is stored in plain text in the database"* auf "Planned" [11]. Beim Fossify Voice Recorder wurde die Bitte um Verschlüsselung (*"People need to be able to at least discourage third parties from accessing recordings"*) als wontfix geschlossen [12], was eine Lücke offen lässt. Apple Journal legt Einträge lokal als RTF in einer SQLite-Datenbank ab, laut dem Kommandozeilenwerkzeug apple-journal-cli *"without encryption at the local level"* [77].

Datenschutz-Details, die Nutzer tatsächlich bemerken, sind kleiner und günstiger als Verschlüsselung: Eine Play-Store-Rezension zu Easy Diary (2. Januar 2022) nennt als einzigen Hinderungsgrund, dass der gerade geschriebene Text *"is still visible in the Recents menu snapshot view"* [74]; bei One Second Diary fragt ein Nutzer, wie er die automatische Sicherung seiner Medien durch Google Fotos verhindert [77]. Und die deutsche Day-One-Klientel begründet ihre Abo-Akzeptanz ausdrücklich damit, dass die Daten *"Ende-zu-Ende verschlüsselt"* seien und Abo einer *"Werbefinanzierung bei persönlichen Daten"* vorzuziehen sei (Store-Snippet, unverifiziert) [18][72].
...
**Konsequenz:** D5 und D14 decken das ab (Envelope-Verschlüsselung, Privacy-Overlay im App-Switcher und FLAG_SECURE, Audiodateien außerhalb der Medienordner, keine SDKs mit Netzzugriff, keine INTERNET-Permission auf Android). Wichtig ist, dass dieser Zustand für Nutzer prüfbar wird: Flugmodus-Test im Onboarding, Datenschutzlabel "Keine Daten erfasst", offener Quellcode (D10).

### 3.4 Transkriptionsqualität für Deutsch und Schweizerdeutsch (Priorität: hoch)

Fünf wiederkehrende Fehlerbilder, alle mit deutschsprachigem Bezug:
...
| Deutsche Rechtschreibregeln im Systemdiktat | *"Dictation in German has a couple of persistent bugs."*: Komposita werden getrennt geschrieben (*"Compound words are either cohesive or hyphenated, the english spelling with two separate words is simply wrong!"*), *"German dictation adds spaces before and after hyphens"*, und *"Anführungszeichen"* wird zugleich als Satzzeichen und als Wort eingefügt | Apple Developer Forums, Thread 826756 (Mai 2026) [78] |

Schweizerdeutsch ist ein eigenes Kapitel. Das Transkriptionsprojekt Transkribor hält fest, dass für Mundart-Aufnahmen die nachgelagerte Korrektur etwa die Hälfte der Arbeit ausmacht, und plant ein optionales Schweizerdeutsch-Modell mit dem Grundsatz *"Erst messen, dann bauen."* [23]. Die Forschungslage: Whisper ohne Anpassung erreicht auf Schweizerdeutsch 23 bis 33 % Wortfehlerrate, feinabgestimmte Modelle 12,1 % bzw. 25,6 % je nach Testdaten (alle unverifiziert, aus Sekundärzusammenfassungen) [24]. Eine Schweizer App-Store-Rezension bemängelt ausdrücklich, dass Schweizerdeutsch nicht erkannt wird [25]. Die beiden öffentlich verfügbaren Whisper-Fine-Tunes, die Transkribor als Kandidaten nennt (apexAI-Switzerland auf large-v3 mit WER 13,31, gated; Flurin17 auf large-v3-turbo, rund 1,6 GB), stehen unter CC-BY-NC-4.0 und sind für eine kommerzielle App ohne eigene Lizenzvereinbarung nicht nutzbar [23][72]; ein Versuch, das Swiss-German-Modell für whisper.cpp zu konvertieren, scheiterte und blieb als "stale" liegen [79].

**Konsequenz:** D2, D3 und D12 sind darauf ausgelegt: feste Sprachwahl statt Auto-Erkennung, `SpeechDetector` bzw. VAD gegen Stille-Halluzinationen, Streaming-Modelle mit Interpunktion, ehrliche Erwartung "Hochdeutsch liefert die zuverlässigsten Ergebnisse". Die Diktat-Bugs betreffen die App direkt, weil der iOS-Fallback `DictationTranscriber` (D2) dieselben Modelle wie das Systemdiktat verwendet; Komposita, Bindestriche und gesprochene Satzzeichen gehören deshalb in das deutsche Testset des Transkriptions-Spikes, und zwar für `SpeechTranscriber`, `DictationTranscriber` und Moonshine getrennt. Eine Präzisierung aus der Faktenprüfung: Apple veröffentlicht keine feste Sprachliste für `SpeechTranscriber`; `supportedLocales` muss zur Laufzeit abgefragt werden, die Ausgabe war über die iOS-26.x-Versionen nicht stabil, und Apple bestätigte im Januar 2026, dass Arabisch fälschlich als unterstützt gelistet war [69][70]. de-DE, de-AT und de-CH stehen in einer auf macOS 26.4 abgefragten Liste, jede Ziel-Locale ist aber auf Zielgerät und Ziel-OS mit eigenem Audio zu prüfen, bevor die App sie anbietet. Und: Das Audio bleibt immer erhalten (A5), das Transkript ist nur die zweite Wahrheit.
...
**Konsequenz:** D3 setzt auf kleine Streaming-Modelle als Standard (Moonshine German Small Streaming, 123 M Parameter, Deutsch-WER 7,5 %, MIT; Faktenprüfung: bestätigt [69]) und macht große Modelle zum optionalen Zusatzpaket. Dass dieses Paket erst ab 6 GB RAM angeboten wird (D13), hat einen konkreten Grund: Für Parakeet TDT 0.6B v3 int8 (rund 640 MB auf der Platte, Dateigrößen laut Faktenprüfung bestätigt) meldet ein Nutzer 1,23 GB RAM beim Laden; das Issue wurde ohne sichtbare Antwort der Maintainer geschlossen, es bleibt eine einzelne Messung, aber ein deutlicher Hinweis, dass der Arbeitsspeicherbedarf die Dateigröße klar übersteigt [69][71]. Das Zusatzpaket hat neben der Genauigkeit einen zweiten Nutzen: Moonshine liefert keine Zeitstempel, Parakeet TDT dagegen Token-Zeitstempel; wortgenaues Mitführen des Textes bei der Wiedergabe gibt es auf Android deshalb erst mit dem Paket, im MVP nur segmentweise (D3, D4) [73]. Der Download des Zusatzpakets muss fortsetzbar sein, und die App muss ohne das Paket vollständig funktionieren. Apple Watch ist kein MVP-Thema.
...
*"Any plans for Android Release?"* ist die Kernfrage an DailyVox (Issue #4, 3. August 2026) [32]; eine Android-Version gibt es nicht [72]. Das wiegt schwerer, seit DailyVox im August 2026 mit v1.10.0 Deutsch (sowie Spanisch, Französisch, Italienisch) hinzugefügt und mit v1.11.0 den Cloud-Fallback vollständig entfernt hat: Fehlt das On-Device-Modell, stoppt die App, statt Audio an Server zu schicken [10][72]. Auf iOS existiert damit ein kostenloser, deutschsprachiger, MIT-lizenzierter Wettbewerber, der vollständig lokal arbeitet. Bei Journiv ist "Mobile app implementation" die meistgewünschte Diskussion, mit dem Argument *"for Wife-Approval-Factor, to be able to journal from an app instead of going to a website"* [33]. Apple Journal, Diarly, Cleft und DailyVox sind Apple-only; Journey transkribiert auf Android gar nicht (siehe `01-markt-und-wettbewerb.md`).

**Konsequenz:** Android ist die größere Lücke und darf nicht der nachgelagerte Port sein (M9, D1, D3). Gegenüber DailyVox auf iOS liegt der Unterschied nicht mehr in "lokal" oder "Deutsch", sondern in Verschlüsselung im Alltag (D5), Backup und QR-Gerätewechsel zwischen den Plattformen (D6), der Erinnerungslogik ohne Schuldgefühl (D7) und der Produktreife; plattformübergreifend in Android selbst.
...
Zu AudioPen: *"$100 insane to be able to do more than just 10 notes"* (unverifiziert) [41]. Zu Day One: *"$25-50 a year for a single purpose app is prohibitively expensive"* (unverifiziert) [42]. In der Journiv-Community löste ein neu aufgetauchter "Plus"-Tab sofort die Frage aus, ob das Projekt *"no longer completely open source"* sei [39]. Dictus formuliert als Grundsatz: *"No bait-and-switch — free features never become paid."* [40]. Free-Tiers mit 10 Einträgen pro Monat (CortexOS) oder 5 pro Woche (EchoVault) verhindern das tägliche Nutzen, das ein Tagebuch ausmacht (unverifiziert) [62][63]. Im deutschen App Store ist Abo gegen Einmalkauf ein ausdrückliches Bewertungskriterium: Zu Diarium heißt es *"Es ist kein Abo! Eine Einmalzahlung von knapp 10 Euro und man kann für immer Tagebuch schreiben"*, zu Day One musste sich ein Rezensent erst *"an das omnipräsente Abo-Modell"* gewöhnen (beides Store-Snippets ohne Datum, unverifiziert) [18][72][80].
...
| 9 | Audio-Wellenform mit Markierungen | Journiv #88 [48] | D4 (Wellenform mit 100 Pegelwerten wird bei der Aufnahme berechnet und verschlüsselt gespeichert); Markierungen offen |
...
|---|---|---|
| DailyVox (iOS, MIT, kostenlos) | "Data Not Collected"-Label, keine Netzwerkaufrufe, seit v1.10.0 (August 2026) Deutsch, seit v1.11.0 ohne Cloud-Fallback, Export in vier Formaten plus AES-Backup, Widgets, Siri Shortcuts [10][72] | ja, alles außer iCloud-Option; zugleich der direkteste Wettbewerber auf iOS (Abschnitt 3.7) |
...
| 2 | **Backup, das man prüfen kann**: eine Containerdatei für Backup und Transfer, sichtbarer Speicherort, manuelle Dateiauswahl beim Wiederherstellen, "Backup prüfen"-Funktion, monatliche Erinnerung, Erklärung der OS-Backup-Grenzen, kein Aussperren nach Biometrie-Änderung | Easy Diary #252/#233/#184/#200/#197, Journiv #540, StoryPad #611 [6][7][9][74][76] | M | MVP (D5, D6) |
| 3 | **Export vor dem ersten Eintrag sichtbar**: Markdown + JSON + M4A (AAC-LC), Datumsbereich wählbar, PDF; Hinweis im Onboarding | Apple Journal "trapped", zehn Konverter [1][2] | S | MVP (D9; Datumsbereich ergänzen) |
...
| 13 | Wellenform mit Lesezeichen im Audio | Journiv #88 [48] | S bis M | Wellenformdaten werden ab MVP bei der Aufnahme erfasst (D4) [73], Anzeige 1.x, Lesezeichen später |
...
|---|---|---|---|
| Export nur in ein proprietäres oder unvollständiges Format, kein Datumsbereich, kein Import | Apple Journal (HTML-ZIP), Journey (Audio nur als MP3) | [1][2][3] | Markdown, JSON, PDF, M4A; Datumsbereich; Import des eigenen Containers (D6, D9) |
...
| Backup-Datei unauffindbar, OS-Update bricht das Backup, keine manuelle Dateiauswahl beim Wiederherstellen | Easy Diary (Play-Rezensionen), billthefarmer/Diary, StoryPad | [74][75][76] | sichtbarer Speicherort und Teilen-Blatt, manuelle Auswahl mit Vorschau, "Backup prüfen" (D6) |
| Aussperrung nach Änderung der Biometrie | Easy Diary #197 | [74] | Gerätecode immer als Rückfall, Passphrase-Wiederherstellung erklärt (D5, D6) |
| Tagebuchtext in der App-Switcher-Vorschau, Medien in der Foto-Sicherung | Easy Diary #186, One Second Diary #97 | [74][77] | Privacy-Overlay, FLAG_SECURE, Audiodateien außerhalb der Medienordner (D5) |
...
| Android als nachgereichter, schwächerer Port | Flint (Funktionsgleichheit unklar), DailyVox (keine Android-Version trotz Nachfrage) | [32][72], siehe 01 | gleicher Funktionsumfang auf beiden Plattformen als Erfolgskriterium (D1) |
...
1. **Nachrecherche mit vollem Netzzugang**: wörtliche Rezensionen (DE/AT/CH) zu Day One, Journey, Diarium, Daylio, Apple Journal, Voice-Diary-Apps in App Store und Google Play; Reddit (r/Journaling, r/privacy, r/de_EDV) und Hacker News zu "voice journal", "local whisper diary" und KI-Ablehnung; deutschsprachige Foren (heise, Kuketz, Android-Hilfe). Der erste Versuch am 3. September 2026 scheiterte an der Proxy-Sperre und lieferte statt der angestrebten 30 deutschen Originalzitate neun Snippet-Fragmente [72]; App-IDs, Paketnamen und Abrufmuster für eine Sitzung ohne Sperre stehen dort in Abschnitt 6. Ziel bleibt: die GitHub-Verzerrung ausgleichen und die Priorisierung in Abschnitt 3 bestätigen oder korrigieren. Erste Stichprobe: Die archivierten Play-Rezensionen stützen Abschnitt 3.2 (Backup) stärker als Abschnitt 3.6 (Erinnerungen), wo Erinnerungen nur als Wunsch, nicht als Unzuverlässigkeits-Beschwerde auftauchten.
...
3. **Schweizerdeutsch**: Wie transkribiert `SpeechTranscriber` mit `de_CH` tatsächlich Mundart-Audio (eigener Test)? Die Faktenprüfung hält fest, dass Apple zu `de-CH` keine Aussage über Dialekterkennung oder Orthografie macht; `de-CH` ist nur ein Standard-Locale-Identifier [69]. Die bekannten Fine-Tunes sind CC-BY-NC-4.0 [23][72]; offen bleibt die Lizenzlage der zugrunde liegenden Trainingsdaten und damit, ob sich ein eigenes, kommerziell nutzbares Modell bauen lässt.
...
72. Nachrecherche Nutzerstimmen und Store-Kennzahlen DE/AT/CH (Stand 3. September 2026): `docs/recherche/nachrecherche-nutzerstimmen-stores.md` in diesem Repository (Abschnitt 1.1 deutschsprachige Snippets und Diktat-Bugs, Abschnitt 1.3 archivierte Play-Store-Rezensionen, Abschnitt 3 Ableitungen, Abschnitt 4 DailyVox v1.10.0/v1.11.0 und Schweizerdeutsch-Lizenzen, Abschnitt 6 Abrufmuster)
73. Nachrecherche Audio-Container, Aufnahme-Pipeline und Wiedergabe-Architektur (Stand 3. September 2026): `docs/recherche/nachrecherche-audio-container.md` in diesem Repository (AAC-LC/M4A statt Opus im MVP, Chunk-AEAD nach Cryptomator-Muster, Moonshine ohne Zeitstempel, Wellenform bei der Aufnahme; Primärquellen dort gelistet)
74. Easy Diary, vom Entwickler archivierte Play-Store-Rezensionen: Issue #184 (28. Dezember 2021, Backup-Speicherort): https://github.com/hanjoongcho/aaf-easydiary/issues/184 ; Issue #200 (11. November 2022, Backup nach Android 13 blockiert): https://github.com/hanjoongcho/aaf-easydiary/issues/200 ; Issue #197 (16. August 2022, Aussperrung nach neuem Fingerabdruck): https://github.com/hanjoongcho/aaf-easydiary/issues/197 ; Issue #111 (22. März 2020, keine manuelle Backup-Auswahl): https://github.com/hanjoongcho/aaf-easydiary/issues/111 ; Issue #186 (2. Januar 2022, App-Switcher-Vorschau): https://github.com/hanjoongcho/aaf-easydiary/issues/186
75. billthefarmer/Diary, Issue #236 "Ho to restore an Backup?" (7. November 2024): https://github.com/billthefarmer/diary/issues/236
76. StoryPad, Issue #611 (11. März 2026, unvollständige Wiederherstellung trotz Google-Drive-Sync): https://github.com/theachoem/storypad/issues/611
77. omarshahine/apple-journal-cli (Apple Journal lokal unverschlüsselt in `moments.sqlite`): https://github.com/omarshahine/apple-journal-cli ; One Second Diary, Issue #97 "How to prevent Google Photos backup" (31. August 2023): https://github.com/KyleKun/one_second_diary/issues/97
78. Apple Developer Forums, Thread 826756 "Dictation in German has a couple of persistent bugs" (Mai 2026): https://developer.apple.com/forums/thread/826756
79. whisper.cpp, Issue #3431 (Konvertierung whisper-large-v3-turbo-swiss-german, 26. September 2025, stale): https://github.com/ggml-org/whisper.cpp/issues/3431
80. App Store DE, Diarium Rezensionen (Snippet, Einmalkauf): https://apps.apple.com/de/app/diarium-privates-tagebuch/id1502834782?mt=12&see-all=reviews&platform=mac
