...
3. Während der Aufnahme zeigt der Bildschirm nur Zeit, Pegel und eine Stopptaste. Aus dem Pegel entsteht nebenbei die Wellenform des Eintrags (100 Werte), die verschlüsselt mit den Metadaten gespeichert wird und später den Player zeichnet; nachträglich ließe sie sich nur durch vollständiges Entschlüsseln und Dekodieren gewinnen (D4) [64]. Eine Live-Vorschau des Texts ist möglich (iOS-Preset `.progressiveTranscription`, D2), aber standardmäßig aus: Sie lenkt vom Sprechen ab und verleitet zum Korrigieren statt Erzählen.
4. Ein Tipp auf "Fertig". Die App verschlüsselt das Audio bereits während der Aufnahme, chunkweise und ohne Klartextdatei (D4), startet die Transkription und kehrt zum Startbildschirm zurück. Der Eintrag erscheint dort mit dem Hinweis "Text wird erstellt", typischerweise nach wenigen Sekunden mit dem fertigen Transkript. Man muss nicht warten.
...
DailyVox ist seit August 2026 mehr als ein Vorbild für Kürze: Version 1.10.0 brachte Deutsch (neben Spanisch, Französisch und Italienisch), Version 1.11.0 entfernte den Cloud-Fallback vollständig. Damit gibt es auf iOS einen kostenlosen, MIT-lizenzierten, deutschsprachigen und vollständig lokalen Wettbewerber; eine Android-Version gibt es nicht, obwohl die Nachfrage dokumentiert ist (Issue #4, 3. August 2026) [65][66]. Für die Positionierung heißt das: Auf iOS unterscheidet sich die App durch Verschlüsselung, automatisches Backup und QR-Gerätewechsel, die Erinnerungslogik (Abschnitt 4) und Produktreife; plattformübergreifend durch Android.

### 2.2 Wiedergabe

Ein Eintrag zeigt Datum, Wochentag, Dauer und Sprache. Darunter der Player mit Wellenform und Fortschrittsbalken, darunter der Text. Die Wiedergabe liest direkt aus der verschlüsselten Datei: Weil jeder 32-KiB-Chunk für sich entschlüsselbar ist, kann man an jede Stelle springen, ohne dass Klartext auf das Dateisystem gelangt (D4). Die Geschwindigkeit lässt sich zwischen 0,8 und 2,0 einstellen, bei gleichbleibender Tonhöhe; die Wahl bleibt über Einträge hinweg erhalten. Tippt man auf ein Wort, springt die Wiedergabe an diese Stelle; läuft die Wiedergabe, wird der gesprochene Abschnitt im Text mitgeführt. Auf iOS ist das wortgenau, weil `SpeechTranscriber` mit dem Attribut `audioTimeRange` je Wort einen Zeitbereich liefert [27]. Auf Android liefert das Standardmodell Moonshine keine Zeitstempel [64][73]; im MVP wird dort absatzweise mitgeführt und gesprungen, wobei die Absätze aus den Segmenten der Sprachaktivitätserkennung entstehen (D3, D4). Wortgenau wird es auf Android erst mit dem Genauigkeitspaket, dessen Modell Parakeet TDT über sherpa-onnx Token-Zeitstempel liefert (Abschnitt 8.2). Wiedergabe funktioniert mit Bildschirm aus und über Kopfhörer; auf Android läuft sie dafür in einem Mediendienst (Media3 `MediaSessionService` mit ExoPlayer), wie es die Hintergrundaudio-Regeln von Android 17 verlangen [67], auf iOS über `AVAudioEngine` mit Anbindung an die Systemsteuerung [68]. Die Bedienung über Sperrbildschirm und Kopfhörer (Play, Pause, 15 Sekunden vor und zurück, Geschwindigkeit) zeigt nur "Tagebuch, 3. September", keinen Text (D5).
...
2. **Automatik.** Nach dem Speichern eines Eintrags prüft die App, ob das letzte Backup älter als sieben Tage ist oder seit dem letzten Backup fünf Einträge hinzugekommen sind (eigene Festlegung, im Beta-Test prüfen). Trifft eines zu, schreibt sie direkt im Anschluss, im Vordergrund, einen vollständigen Container in den Ordner. Das braucht keine Hintergrundplanung und verhält sich auf beiden Plattformen gleich. Vollständige statt inkrementelle Backups sind vertretbar: Bei 3 Minuten Sprache täglich und AAC-LC mit 32 kbit/s wächst das Tagebuch um rund 263 MB pro Jahr (D4, D6; auf dem späteren Opus-Pfad 131 bis 197 MB); die App behält die letzten drei Dateien und löscht ältere, also nach eigener Rechnung rund 0,8 GB am Ende des ersten Jahres. Ob das wöchentliche Schreiben einiger hundert Megabyte in einen Anbieter-Ordner im Vordergrund zumutbar bleibt, prüft der Beta-Test (offene Frage 10); ab dem zweiten Jahr wäre ein inkrementelles Format die erste Optimierung.
...
| **Eintrag** | Datum, Dauer, Sprach-Badge (de-DE/de-AT/de-CH/en); Player (Wellenform, Geschwindigkeit 0,8 bis 2,0); Transkript mit Tipp-zum-Springen (iOS wortgenau, Android absatzweise, Abschnitt 2.2); "Ergänzen" (weitere Aufnahme); Text bearbeiten; erneut transkribieren, falls die Erkennung fehlgeschlagen ist; Audio dieses Eintrags löschen; Eintrag löschen; Eintrag exportieren. Ab 1.1: Sprache ändern und neu transkribieren, Original-Transkript wiederherstellen. | Audio und Text gleichwertig (M5, A5); Korrektur in der Hand der Nutzerin. |
| **Kalender und Suche** | Monatsraster mit markierten Tagen; Liste nach Datum; Suchfeld mit Filtern (Zeitraum, Sprache, nur mit Audio). Ab 1.1: Monatszahlen (Tage, Minuten, Wörter), "Zufälliger Eintrag". | Volltextsuche ist bei Sprachnotiz-Apps der meistgewünschte Zusatz (Fossify Voice Recorder Issue #34, siehe `02-nutzerfeedback-und-luecken.md`, Abschnitt 4); Heatmap statt Streak [5]. |
...
**Anzeige.**
- Der Text steht als Fließtext in Absätzen, die aus Sprechpausen gebildet werden. Wortweise Zeitmarken erlauben auf iOS Tipp-zum-Springen und wortgenaues Mitführen [27]. Auf Android liefert Moonshine keine Zeitstempel [64][73]; im MVP trägt dort jeder Absatz den Zeitstempel seines VAD-Segments, ein Tipp auf den Absatz springt an dessen Anfang, und beim Abspielen wird der Absatz hervorgehoben (D3). Wortgenau wird es auf Android mit dem Genauigkeitspaket: Parakeet TDT liefert über sherpa-onnx Token-Zeitstempel, die zu Wörtern zusammengeführt werden müssen (Abschnitt 8.2, offene Frage 2). Spike S2 (`09-roadmap.md`) misst die Genauigkeit dieser Zeitstempel auf deutscher Sprache.
- Ein Sprach-Badge zeigt, mit welcher Locale transkribiert wurde. Die Sprache wird nicht automatisch erkannt, sondern ist eine Einstellung, änderbar pro Eintrag: Automatische Spracherkennung führt bei deutsch-englischen Mischsätzen zu Sprachwechseln oder Übersetzungen statt Transkription (whisper.cpp #1800, easyspeak #153, siehe `02-nutzerfeedback-und-luecken.md`, Abschnitt 3.4).
- Wörter mit niedriger Konfidenz werden dezent unterlegt (iOS liefert `transcriptionConfidence` je Ergebnis [27]; Android: offene Frage 2). Kein rotes Markieren, kein "Fehlerzähler".
...
- Nachbearbeitung für Groß-/Kleinschreibung und Interpunktion, falls das Modell sie nicht liefert; deutsche Kleinschreibung durchgehend ist ein bekanntes Ärgernis [33]. Selbst Apples Diktat trennt deutsche Komposita in Einzelwörter und setzt Leerzeichen um Bindestriche (Apple Developer Forums, Mai 2026) [71]; der Beta-Test prüft die gewählten Modelle auf dieselben Muster, und eine regelbasierte Korrektur der Bindestrich-Leerzeichen ist billig.
- de-CH bedeutet Schweizer Standarddeutsch, nicht Mundart; Apple veröffentlicht keine feste Liste, `supportedLocales` ist die Wahrheit [28][29]. Dass die Erkennung für de-CH auch Schweizer Orthografie (ss statt ß) ausgibt, belegt keine Apple-Quelle; das ist plausibel, aber im Spike zu prüfen (Faktenprüfung Eintrag 2 [70]), notfalls ersetzt die Nachbearbeitung ß für de-CH. Die App sagt das im Onboarding und beim Sprach-Badge offen (D12). Mundart-Modelle existieren nur als Fine-Tunes unter CC-BY-NC-4.0, also nicht kommerziell nutzbar [53][65]; siehe Abschnitt 8, "später".
...
| Lokale Transkription Deutsch (de-DE/de-AT/de-CH) und Englisch, feste Sprachwahl; iOS 26 ab iPhone 12 empfohlen, Android 12 ab 4 GB RAM (D13) | M4, M6, A1, D2, D3, D12; Apple Journal transkribiert nur Englisch [1], Day One nur bis 10 Minuten und bis iOS 26 nur online [2]; iPhone 11 und SE 2 bieten `SpeechTranscriber` nicht, dort greift der Fallback [30] (Faktenprüfung Eintrag 3 [70]) | kostenlos |
| Wiedergabe direkt aus der verschlüsselten Datei mit Wellenform, Geschwindigkeit 0,8 bis 2,0 und Steuerung über Sperrbildschirm und Kopfhörer; Text lesen und mitführen: wortgenau auf iOS [27], absatzweise (VAD-Segmente) auf Android | M5, A5, D4; Moonshine liefert keine Zeitstempel [64], wortgenau auf Android erst mit dem Genauigkeitspaket (Abschnitt 8.2, Frage 2) | kostenlos |
...
| Verschlüsselung, App-Sperre per Biometrie/Gerätecode (Standard 30 s, einstellbar bis 5 Minuten; der Gerätecode bleibt immer als Rückfall, damit ein neuer Fingerabdruck niemanden aussperrt), Privacy-Overlay | M7, D5; Verschlüsselung ist bei Tagebuch-Apps die meistgefragte Eigenschaft (Recherche Nutzerfeedback); Apple Journal legt Einträge lokal unverschlüsselt in SQLite ab [72]; Aussperrung nach Biometrie-Änderung und sichtbare App-Switcher-Vorschauen sind dokumentierte Store-Beschwerden [69] | kostenlos |
| Audio behalten oder löschen, pro Eintrag und global; Speicheranzeige | A5, D4; Rechenbasis: 3 Minuten täglich ergeben bei AAC-LC 32 kbit/s 0,72 MB pro Tag und rund 263 MB pro Jahr (D4, D6; späterer Opus-Pfad 131 bis 197 MB); die Speicheranzeige nennt Audio und Text getrennt | kostenlos |
| Automatisches verschlüsseltes Backup in einen selbst gewählten Ordner (Passphrase, Wiederherstellungstest, "Letztes Backup vor X Tagen"); zusätzlich manuelles Backup als Datei; Wiederherstellung im Onboarding | A8, D6; Abschnitt 2.4; automatisches Backup ist der ausdrückliche Nutzerwunsch [59]; unauffindbare Backup-Dateien, durch OS-Updates gebrochene Backups und unvollständige Wiederherstellungen sind die häufigsten echten Store-Beschwerden [65][69] | kostenlos |
...
| Export: Markdown, JSON, Audio als M4A (AAC-LC, überall abspielbar), jeweils das ganze Tagebuch; vor dem ersten Eintrag ausprobierbar | Lock-in ist der Hauptvorwurf gegen Apple Journal, das nur HTML exportiert [46]; Markdown, JSON und M4A reichen als Nachweis, dass nichts eingesperrt ist (D4) | kostenlos |
...
| Genauigkeitspaket "Hohe Genauigkeit" (iOS WhisperKit large-v3-v20240930, 626 MB, ab iPhone 13/A15; Android Parakeet TDT 0.6B v3 int8 ≈ 640 MB, ab 6 GB RAM); auf Android bringt es zugleich Token-Zeitstempel für wortgenaues Mitführen | D2, D3, D13; Deutsch-WER ≈ 4 % (Parakeet, Leaderboard-Rohdaten Stand 26. 3. 2026, aggregiert 5,0 %) statt 7,5 % beim Standardmodell [31] (Faktenprüfung Einträge 5, 25 und 28 [70]) | Plus |
...
- **Kein Versprechen für Schweizerdeutsche Mundart** (D12), keine Transkription auf der Uhr [21], kein Wear-OS-Mikrofon über den Data Layer (Abschnitt 5), kein Betrieb, der nie eine Netzverbindung braucht: Der Modell-Download durch das Betriebssystem bzw. den Play Store ist unvermeidlich und wird erklärt; auf iOS aktualisiert das System die Modelle später selbst, "einmalig" gilt also nur für den ersten Download (D14; Faktenprüfung Eintrag 1 [70]).
...
2. **Zeitmarken und Konfidenz auf Android.** Teilweise geklärt: Moonshine liefert über sherpa-onnx keine Zeitstempel, Parakeet TDT liefert Token-Zeitstempel [64][73]; das MVP führt deshalb absatzweise mit (Abschnitt 2.2). Offen bleiben: Wie genau sind die Parakeet-Zeitstempel auf deutscher Sprache, nachdem BPE-Token zu Wörtern zusammengeführt sind (Spike S2)? Liefert eines der Android-Modelle Konfidenzwerte, mit denen sich unsichere Wörter unterlegen lassen? Und reicht die VAD-Segmentierung für Absätze, die beim Lesen natürlich wirken?
...
13. **Verifikation.** Die Faktenprüfung liegt vor (`docs/recherche/verifikation.json`, 30 Aussagen: 18 bestätigt, 12 präzisiert) [70]; ihre Präzisierungen sind eingearbeitet (Modell-Assets, de-CH-Orthografie, WhisperKit-Gerätegrenze, Live-Activity-Start, Watch-Benchmark, WER-Zahlen). Als "unverifiziert" gekennzeichnet bleiben nur Angaben, die nicht Gegenstand der Prüfung waren: die Retention-Zahl [49], die Abend-Check-in-Studie [8], die Duolingo-Zahlen [7], Diktat und Legasthenie [42], die BFSG-Fassung [41], die Einordnung nach AI Act und MDCG [44][45] sowie die Store-Kennzahlen der Nachrecherche [65], die dort selbst als `mustVerify` markiert sind. Diese Punkte sind vor Veröffentlichung zu prüfen (D15).
...
64. Nachrecherche Audio-Container, Aufnahme-Pipeline und Wiedergabe (AAC-LC/M4A statt Opus, Chunk-AEAD nach Cryptomator-Muster, Moonshine ohne Zeitstempel, Wellenform bei der Aufnahme): `docs/recherche/nachrecherche-audio-container.md`
65. Nachrecherche Nutzerstimmen und Store-Kennzahlen DE/AT/CH (Abschnitt 4: DailyVox Deutsch und ohne Cloud-Fallback; Store-Beschwerden zu Backups; Schweizerdeutsch-Fine-Tunes CC-BY-NC): `docs/recherche/nachrecherche-nutzerstimmen-stores.md`
66. DailyVox, README mit Versionshistorie (v1.10.0 Deutsch, v1.11.0 ohne Cloud-Fallback, August 2026) und Issue #4 (Android-Nachfrage, 3. August 2026): https://github.com/intrepidkarthi/dailyvox ; https://github.com/intrepidkarthi/dailyvox/issues/4
67. Android 17, Background Audio Hardening; Media3, Hintergrundwiedergabe mit `MediaSessionService`: https://developer.android.com/about/versions/17/changes/bg-audio ; https://developer.android.com/media/media3/session/background-playback
68. Apple, `MPRemoteCommandCenter` (Sperrbildschirm- und Kopfhörersteuerung) und `AVAudioUnitTimePitch.rate` (Geschwindigkeit ohne Tonhöhenänderung): https://developer.apple.com/documentation/mediaplayer/mpremotecommandcenter ; https://developer.apple.com/documentation/avfaudio/avaudiounittimepitch/rate
69. Easy Diary, vom Entwickler archivierte Play-Store-Rezensionen: #184 (Backup-Datei nicht auffindbar), #200 (Backup nach Android-13-Update blockiert), #197 (Aussperrung nach neuem Fingerabdruck), #186 (Inhalt in der App-Switcher-Vorschau sichtbar), #111 (Backup-Datei nicht wählbar): https://github.com/hanjoongcho/aaf-easydiary/issues/184 ; https://github.com/hanjoongcho/aaf-easydiary/issues/200 ; https://github.com/hanjoongcho/aaf-easydiary/issues/197 ; https://github.com/hanjoongcho/aaf-easydiary/issues/186 ; https://github.com/hanjoongcho/aaf-easydiary/issues/111
70. Faktenprüfung von 30 Aussagen aus den Rechercheberichten (18 bestätigt, 12 präzisiert; im Text nach Eintragsnummer zitiert): `docs/recherche/verifikation.json`
71. Apple Developer Forums, Thread 826756 (deutsche Diktat-Fehler: Komposita getrennt, Leerzeichen um Bindestriche, Interpunktionswörter doppelt; Mai 2026): https://developer.apple.com/forums/thread/826756
72. apple-journal-cli (Apple Journal speichert Einträge lokal als RTF in `moments.sqlite` ohne Verschlüsselung): https://github.com/omarshahine/apple-journal-cli
73. sherpa-onnx, Quellcode der Offline-Erkenner: Moonshine-Implementierung ohne Zeitstempel, Transducer-Implementierung (Parakeet TDT) mit Token-Zeitstempeln: https://github.com/k2-fsa/sherpa-onnx/blob/master/sherpa-onnx/csrc/offline-recognizer-moonshine-impl.h ; https://github.com/k2-fsa/sherpa-onnx/blob/master/sherpa-onnx/csrc/offline-recognizer-transducer-impl.h
